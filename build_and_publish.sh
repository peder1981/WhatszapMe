...
build_linux() {
  ...
  cd "$(dirname "$0")/cmd/whatszapme-gui" || exit 1
  
  # Verificar se há erro no código antes de compilar
  if ! go build -o /tmp/check_build_linux; then
  ...
  # Compilar e empacotar com Fyne
  if ! fyne package -os linux -icon ../../assets/icon.png -name WhatszapMe --app-id com.peder.whatszapme; then
...
run_tests() {
  ...
  if ! go test -timeout 30s $packages -v; then
...
move_builds() {
  ...
  # Linux
  if [ -f "$(dirname "$0")/cmd/whatszapme-gui/WhatszapMe.tar.xz" ]; then
    cp -v "$(dirname "$0")/cmd/whatszapme-gui/WhatszapMe.tar.xz" "$build_dir/WhatszapMe_Linux_$timestamp.tar.xz"
    if [ $? -eq 0 ]; then
    ...
    warn "Arquivo WhatszapMe.tar.xz não encontrado"
  fi
  
  # Windows
  if [ -f "$(dirname "$0")/cmd/whatszapme-gui/WhatszapMe.exe" ]; then
    cp -v "$(dirname "$0")/cmd/whatszapme-gui/WhatszapMe.exe" "$build_dir/WhatszapMe_Windows_$timestamp.exe"
    if [ $? -eq 0 ]; then
    ...
    warn "Arquivo WhatszapMe.exe não encontrado"
  fi
  
  # macOS
  if [ -f "$(dirname "$0")/cmd/whatszapme-gui/WhatszapMe-macOS" ]; then
    cp -v "$(dirname "$0")/cmd/whatszapme-gui/WhatszapMe-macOS" "$build_dir/WhatszapMe_macOS_$timestamp"
    if [ $? -eq 0 ]; then
    ...
    warn "Arquivo WhatszapMe-macOS não encontrado"
  fi
